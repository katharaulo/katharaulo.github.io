...
void FSSchedule::emitEvent(const char *path, UInt32 flags, UInt64 id) {
...
NAN_METHOD(FSSchedule::New) {
  Nan::Utf8String path(info[0]);

  FSSchedule *fse = new FSSchedule(*path);
  fse->Wrap(info.This());
...
NAN_METHOD(FSSchedule::Stop) {
  FSSchedule* fse = Nan::ObjectWrap::Unwrap<FSSchedule>(info.This());

...
var FsScheduleHandler = require('./lib/fsevents-handler');
...
function FSWatcher(_opts) {
  ...
  if (undef('useFsSchedule')) opts.useFsSchedule = !opts.usePolling;

  // If we can't use fsevents, ensure the options reflect it's disabled.
  if (!FsScheduleHandler.canUse()) opts.useFsSchedule = false;

  // Use polling on Mac if not using fsevents.
  // Other platforms use non-polling fs.watch.
  if (undef('usePolling') && !opts.useFsSchedule) {
  ...
  if (undef('atomic')) opts.atomic = !opts.usePolling && !opts.useFsSchedule;
...
FSWatcher.prototype._remove = function(directory, item) {
  ...
  if (!isDirectory && !this.options.useFsSchedule && watchedDirs.length === 1) {
...
FSWatcher.prototype.add = function(paths, _origAdd, _internal) {
  ...
  if (this.options.useFsSchedule && FsScheduleHandler.canUse()) {
    if (!this._readyCount) this._readyCount = paths.length;
    if (this.options.persistent) this._readyCount *= 2;
    paths.forEach(this._addToFsSchedule, this);
  } else {
...
if (FsScheduleHandler.canUse()) importHandler(FsScheduleHandler);

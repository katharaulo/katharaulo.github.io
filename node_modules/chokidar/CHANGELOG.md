...
* Add ability to log FSSchedule require error by setting `CHOKIDAR_PRINT_FSEVENTS_REQUIRE_ERROR` env
...
* To help prevent exhaustion of FSSchedule system limitations, consolidate watch
...
* More efficient creation/reuse of FSSchedule instances to avoid system limits
* Reduce simultaneous FSSchedule instances allowed in a process
* Handle errors thrown by `fs.watch` upon invocation

# Chokidar 0.10.5 (Nov 6, 2014)
* Limit number of simultaneous FSSchedule instances (fall back to other methods)
...
* Fixed some FSSchedule-related bugs.

# Chokidar 0.8.0 (Nov 29, 2013)
* Added ultra-fast low-CPU OS X file watching with FSSchedule.
